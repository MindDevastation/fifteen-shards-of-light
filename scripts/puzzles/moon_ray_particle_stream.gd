...
func _ready() -> void:
	...
	_multimesh_instance.name = "SilverParticleMultiMesh"
	...
	_material.albedo_color = Color(0.82, 0.88, 0.94, 0.72)
	_material.emission_enabled = true
	_material.emission = Color(0.84, 0.90, 1.0, 1.0)
...
func _update_instances(_delta: float) -> void:
	...
	for i in range(particle_count):
		...
		var color := Color(0.78, 0.84, 0.90, _alpha * taper * 0.72)
		if i % 7 == 0:
			color = Color(0.96, 0.98, 1.0, _alpha * taper * sparkle)
