extends Node3D
class_name MoonRayPuzzleController

signal puzzle_completed

const LanternScript := preload("res://scripts/puzzles/moon_ray_lantern_node.gd")
const StreamScript := preload("res://scripts/puzzles/moon_ray_particle_stream.gd")
...
var _lanterns: Array[MoonRayLanternNode] = []
var _streams: Array[MoonRayParticleStream] = []
var _current_index := 1
var _selected: MoonRayLanternNode
var _locked := false
...
func _ready() -> void:
	...
	_stream_root.name = "MoonRayRuntimeParticleStreams"
...
func reset_to_initial() -> void:
	...
	for i in range(_lanterns.size()):
		if i == 0:
			_lanterns[i].set_lantern_state(MoonRayLanternNode.LanternState.COMPLETED)
		elif i == 1:
			_lanterns[i].set_lantern_state(MoonRayLanternNode.LanternState.ACTIVE_ENDPOINT)
		else:
			_lanterns[i].set_lantern_state(MoonRayLanternNode.LanternState.INACTIVE)
	_create_stream(0, 1, true)
	_refresh_prompts()

func can_lantern_be_interacted(lantern: MoonRayLanternNode) -> bool:
...
func get_prompt_text_for_lantern(lantern: MoonRayLanternNode) -> String:
	...
	if _selected != null and can_lantern_be_interacted(lantern):
		return "Соединить лунный луч"
	return "Направить лунный луч"
...
func _bind_lanterns() -> void:
	...
	for path in lantern_paths:
		...
		if visual == null:
			push_error("BLOCKER: required Moon Ray lantern node is missing: %s" % [path])
			continue
	...
	if _lanterns.size() != 5:
		push_error("BLOCKER: Moon Ray requires exactly five lantern nodes; found %d" % _lanterns.size())

func _on_lantern_interaction(lantern: MoonRayLanternNode, _player: Node) -> void:
	if not can_lantern_be_interacted(lantern):
		lantern.play_invalid_feedback()
		return
	if _selected == lantern:
		_selected = null
		lantern.set_lantern_state(MoonRayLanternNode.LanternState.ACTIVE_ENDPOINT)
		_refresh_prompts()
		return
	if _selected == null:
		_selected = lantern
		lantern.set_lantern_state(MoonRayLanternNode.LanternState.SELECTED)
		_refresh_prompts()
		return
...
func _try_connect(target: MoonRayLanternNode) -> void:
	...
	_selected.set_lantern_state(MoonRayLanternNode.LanternState.COMPLETED)
	_selected = null
	if target_index == source_index + 1:
		target.play_success_feedback()
		_current_index = target_index
		if _current_index == _lanterns.size() - 1:
			target.set_lantern_state(MoonRayLanternNode.LanternState.COMPLETED)
			_complete_puzzle()
		else:
			target.set_lantern_state(MoonRayLanternNode.LanternState.ACTIVE_ENDPOINT)
		_refresh_prompts()
	else:
		target.set_lantern_state(MoonRayLanternNode.LanternState.RESETTING)
		_start_wrong_reset(stream)

func _create_stream(source_index: int, target_index: int, initial: bool) -> MoonRayParticleStream:
	var stream := StreamScript.new() as MoonRayParticleStream
	stream.name = "moon_ray_particle_stream_%s_to_%s" % [_lanterns[source_index].lantern_id, _lanterns[target_index].lantern_id]
...
func _start_wrong_reset(_wrong_stream: MoonRayParticleStream) -> void:
	_locked = true
	_refresh_prompts()
	...
	for i in range(1, _lanterns.size()):
		_lanterns[i].set_lantern_state(MoonRayLanternNode.LanternState.RESETTING)
