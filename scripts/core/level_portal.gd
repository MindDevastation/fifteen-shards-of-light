...
func _start_transition(player: Node) -> void:
	...
	if result != OK:
		...
		return
	if transition.has_signal(&"transition_failed") and not transition.transition_failed.is_connected(failed_callback):
		transition.transition_failed.connect(failed_callback, CONNECT_ONE_SHOT)
	if transition.has_signal(&"transition_finished") and not transition.transition_finished.is_connected(finished_callback):
		transition.transition_finished.connect(finished_callback, CONNECT_ONE_SHOT)
